...
public class AverageAcceleration2 : MonoBehaviour
{
    ...
    public void ShowPanel(float initialSpeed, float finalTime, float finalSpeed, float answer)
    {
        ...
        instructionText.text = $"Um bloco com velocidade inicial de {initialSpeed}m/s após {finalTime} s, atinge a velocidade final {finalSpeed} m/s. Qual é a aceleração média desse bloco em m/s²?";
        answerInput.text = "";
        feedbackText.text = "";
    ...
    private void OnConfirmButtonClicked()
    {
        ...
        if (float.TryParse(answerInput.text, out float playerAnswer))
        {
            if (playerAnswer == correctAnswer)
            {
                ...
                HidePanel();
                GameManager.Instance.UiManager.tipsAverageAcceleration2.HidePanelTips();
                GameManager.Instance.UiManager.smallTipAverageAcceleration2.HidePanelTips();
            }
            else
            {
